# CAST AI PDB Controller Helm Chart v0.1.2

A Helm chart for deploying the CAST AI Pod Disruption Budget (PDB) Controller v0.2.0 to Kubernetes clusters.

**New in v0.1.2:**
- Enhanced exclusion rules system with regex support
- Configurable intervals for various operations
- Poor PDB detection and automatic fixing capabilities
- Multiple PDB conflict resolution
...
The controller supports exclusion rules to automatically skip PDB creation for specific workloads based on namespace, name, and label patterns. This is useful for system workloads, temporary deployments, or critical services that should not have PDBs.

#### Exclusion Rule Structure

Each exclusion rule can specify:
- **`namespaceRegex`**: Regular expression to match namespace names (empty string = no filter)
- **`nameRegex`**: Regular expression to match workload names (empty string = no filter)  
- **`labels`**: Map of label keys to value regex patterns (empty object = no filter)
...
config:
  exclusions:
    # Exclude all workloads in kube-system namespace
    - namespaceRegex: "^kube-system$"
      nameRegex: ""
      labels: {}
    
    # Exclude temporary workloads with specific labels
    - namespaceRegex: ""
      nameRegex: ".*-temp$"
      labels:
        app: "^db-.*"
    ...
    # Exclude critical workloads in monitoring namespace
    - namespaceRegex: "monitoring"
      nameRegex: ""
      labels:
        role: "critical"
```
...
- **Rule Evaluation**: Each rule is evaluated independently
- **Match Logic**: If a workload matches ANY rule, it is excluded from PDB creation
- **Criteria Matching**: Within a single rule, ALL specified criteria must match (AND logic)
- **Regex Support**: All patterns use Go's regexp package for flexible matching
- **Empty Values**: Empty strings or empty objects mean "no filter" for that criterion

**Note**: Exclusion rules take precedence over all other PDB creation logic, including annotations and default configurations.
...
# Install with exclusion rules
helm install castai-pdb-controller castai/castai-pdb-controller \
  --set config.exclusions[0].namespaceRegex="^kube-system$" \
  --set config.exclusions[0].nameRegex="" \
  --set config.exclusions[0].labels="{}" \
  -n castai-agent --create-namespace
```
